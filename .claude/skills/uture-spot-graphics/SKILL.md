...
**모션/시퀀스/글로우: 오렌지 리드 + 7색 파스텔 로테이션 (주황 단독 금지).**
장면·탭·카드마다 서로 다른 파스텔 틴트(sky/lilac/amber/mint/peach/rose)를 돌려 쓰고, 오렌지는 최종 강조 포인트로만. 토큰은 `web/src/styles/globals.css` `:root`의 `--c-*` 7색만 사용.
