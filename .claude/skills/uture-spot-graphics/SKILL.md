...
**예외 — 히어로급 "프로스트 글라스 체인"**: 트웰브랩스 시그니처의 유쳐 번역.
**최종 구현은 Remotion 렌더 영상** (`motion/src/HeroChain.tsx` →
`web/public/assets/motion/hero-chain.mp4`, 임베드는 `graphics/HeroChainVideo.tsx`).
JS 실시간 버전도 가능했지만 사용자가 영상을 선호("영상을 만들어 넣는 게 낫지 않을까") —
시네마틱 앰비언트는 영상이 맞다: 아트 디렉션 완전 통제, 런타임 비용 0.
핵심: **배경색(#f5f5f5)을 구워 H.264로** 렌더하면 알파가 필요 없어 iOS에서도 재생된다
(알파 WebM 금지 규칙과 충돌 없음). 심리스 루프 = 모든 주기 항을 정수 사이클로.
한글 태그는 DOM 오버레이로 영상 위에 얹는다(압축 뭉갬 방지). 포스터 스틸을
`remotion still`로 함께 렌더해 로딩/reduced-motion 폴백으로.
1차 시도(흩어진 블러 캡슐)는 "얼룩"으로 실패 — 레퍼런스와 직접 비교해 확정한 성립 조건:
