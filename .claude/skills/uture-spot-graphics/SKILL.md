...
**예외 — 히어로급 "프로스트 글라스 비드 체인"** (`graphics/FlowChain.tsx`): 트웰브랩스
시그니처의 유쳐 번역. 블러 덩어리가 유치해지지 않는 조건은 ①스케일(비드 150~240px,
화면을 감는 베지어 곡선) ②무채색 기조 + 악센트 1~2개만 ③크기별 블러(피사계심도)
④한 방향 광원의 유리 음영 그라디언트. 전부 CSS — 이미지 생성/에셋 불필요.
타임스탬프 태그 문법은 유쳐 의미(도구명 · 운영중 칩)로 번역해 붙인다.
