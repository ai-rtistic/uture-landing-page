...
### 1.4 라운드 · 보더 · 섀도우 (consistent token scale)
라운드는 **딱 4개 토큰만** 쓴다(임의 값 금지):
- `--r-pill: 7px` — 라벨·태그·배지 (모든 작은 라벨류)
- `--r-node: 10px` — 그래픽 노드·타일
- `--r-btn: 14px` — 버튼·탭
- `--r-card: 22px` — 카드·그래픽 프레임·영상 프레임

- 보더: 1px solid `#e0e0e0`(헤어라인) / 강조 아웃라인은 `#1d1c1b`.
- 버튼 섀도우(히어로 CTA): `inset 0 0 0 1px rgba(0,0,0,0.1)`.
- 카드 호버: `0 24px 40px -28px rgba(29,28,27,0.4)`.

### 1.5 라벨 시스템 (일관성 핵심)
배지·태그·노드캡션 등 "작은 라벨"은 **두 등급으로만** 만든다. 컴포넌트마다 따로 만들지 않는다.
| 클래스 | 용도 | 보더 | 색 |
|---|---|---|---|
| `.pill` | 섹션 아이브로우/강조 배지 | 1px `#1d1c1b`(다크) | text |
| `.tag` | 보조 메타 라벨(프로세스 태그·노드 태그·기간·캡션) | 1px `#e0e0e0`(회색) | text-2 |

둘 다 동일 DNA: radius **7px** · font **12px** · LH 18–20 · LS **0.25px** · padding **2.5px 6px 1px** · 배경 투명.
→ `narrative-num`, `process-tag`, `pipe-node-tag`, `pipe-cap`, `g-tile-tag`, `pipe-tool` 전부 이 DNA로 수렴.
